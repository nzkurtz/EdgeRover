...
def main():
    ...
    try:
        while True:
            ...
            # Convert raw model output into one observed label for this frame
            if gestures:
                g = gestures[0]
                if g == "None":
                    observed_label = None
                    ...
                    observed_label = f"{handedness}: {g}"
            else:
                observed_label = None

            # Update candidate
            if observed_label == candidate_label:
                candidate_count += 1
            else:
                candidate_label = observed_label
                candidate_count = 1

            # Promote candidate to stable label only after enough consecutive frames
